...
namespace AxarDB.Definitions
{
    public class Collection
    {
        ...
        public void Insert(Dictionary<string, object> document, CancellationToken cancellationToken = default, bool bypassSystemRules = false)
        {
            ...
            // Preserve caller-supplied _id (e.g. from recovery queries) or generate a new GuidV7
            if (!document.TryGetValue("_id", out var existingIdObj) || string.IsNullOrWhiteSpace(existingIdObj?.ToString()))
            {
            ...
            if (Name.Equals("sysusers", StringComparison.OrdinalIgnoreCase))
            {
                if (document.TryGetValue("username", out var userObj) && userObj != null)
                {
                    ...
                    var existingUser = FindAll(cancellationToken).FirstOrDefault(d => 
                        d.TryGetValue("username", out var u) && 
                        string.Equals(u?.ToString(), username, StringComparison.OrdinalIgnoreCase));

                    if (existingUser != null)
                    {
                        string existingId = existingUser.TryGetValue("_id", out var eid) ? eid.ToString()! : "";
        ...
        public void UpdateExisting(Dictionary<string, object> document, Dictionary<string, object> oldDocument, CancellationToken cancellationToken = default, bool bypassSystemRules = false)
        {
            ...
            if (Name.Equals("sysusers", StringComparison.OrdinalIgnoreCase))
            {
                if (document.TryGetValue("username", out var userObj) && userObj != null)
                {
                    ...
                    var duplicate = FindAll(cancellationToken).FirstOrDefault(d => 
                        d.TryGetValue("username", out var u) && 
                        string.Equals(u?.ToString(), username, StringComparison.OrdinalIgnoreCase) &&
                        d.TryGetValue("_id", out var eid) && 
                        !string.Equals(eid?.ToString(), id, StringComparison.OrdinalIgnoreCase));
